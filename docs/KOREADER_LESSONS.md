...
| Power: suspend/standby management, low-battery handling | ✅ power button and magnetic sleep cover suspend to RAM (`gideon-device/src/power.rs`). The dance is KOReader's `Kobo:_doSuspend` verbatim: skip while charging (an MTK suspend with the charger in hangs the kernel), `1 > /sys/power/state-extended`, 2 s settle, `sync`, `mem > /sys/power/state` (blocks until wake), then `0 > state-extended` + 100 ms on resume; `EBUSY` retries reset state-extended like Nickel's own 1→mem→0 loop. Buttons come from a capability scan of `/dev/input/event0..9` for `EV_KEY` 116 (power) / 59 / 35 (covers), `poll(2)`-merged with the touch stream. Reading progress is flushed to disk *before* suspending; wake repaints in full and drops the key press that woke us. Low-battery auto-shutdown still 📋 |
