...
impl Source {
    /// Loads a source from an `.aix` file.
    ///
    /// `settings_dir` is a plain directory where per-source settings written
    /// by the source (through the `defaults` imports) are persisted as JSON.
    pub fn from_aix_file(path: &Path, settings_dir: &Path) -> Result<Self> {
        Self::from_aix_file_with_settings(path, settings_dir, Settings::default())
    }

    pub fn from_aix_file_with_settings(
        path: &Path,
        settings_dir: &Path,
        settings: Settings,
    ) -> Result<Self> {
        #[cfg(feature = "all")]
        let mut blocking_source =
            BlockingSource::from_aix_file(path, settings_dir, &settings, None)?;
        ...
        #[cfg(not(feature = "all"))]
        let blocking_source = BlockingSource::from_aix_file(path, settings_dir, &settings, None)?;
...
impl BlockingSource {
    pub fn from_aix_file(
        ...
        let settings_path = settings_dir.join(format!("{}.settings.json", manifest.info.id));
        let stored_source_settings = {
            ...
            // Settings previously persisted by `SourceSettings::save` take
            // precedence over the ones passed in through `Settings`.
            if let Ok(contents) = fs::read_to_string(&settings_path) {
                if let Ok(from_disk) =
                    serde_json::from_str::<HashMap<String, SourceSettingValue>>(&contents)
                {
                    stored.extend(from_disk);
                }
            }
            ...
            .with_context(|| format!("failed creating instance from {}", path.display()))
        {
            ...
            Err(error) => {
                if force_mode.is_none() {
                    ...
                    return Self::from_aix_file(
                        path,
                        settings_dir,
                        settings,
                        Some(!aidoku_sdk_next),
                    );
                }
